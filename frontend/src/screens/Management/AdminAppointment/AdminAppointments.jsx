...
import { Container, Row, Col, Table, InputGroup, FormControl, Button } from 'react-bootstrap'; // Import Bootstrap components
...
const AdminAppointments = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("http://localhost:3000/api/appointments/getAllAppointmentsAdmin");
  ...
  const deleteAppointment = async (appointmentId) => {
    try {
      const response = await axios.put(`http://localhost:3000/api/appointments/appointmentdelete/${appointmentId}`);
            ...
            <Table striped bordered hover responsive className="text-center">
              ...
              <tbody>
                {filteredAppointments.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan="7">No appointments found</td> {/* Adjusted colSpan based on the number of columns */}
                  </tr>
                )}
              </tbody>
            </Table>
